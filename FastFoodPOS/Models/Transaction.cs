...
namespace FastFoodPOS.Models
{
    class Transaction
    {
        ...
        public void Save()
        {
            ...
            string path = "Transaction\\Transaction.txt";
            ...
            if (!File.Exists(path))
            {
                ...
                {
                    tw.WriteLine("Transaction File");
                    tw.WriteLine("==========================");
                    tw.WriteLine("Transaction ID:  "+ Id);
                    tw.WriteLine("Cashier Id:  "+ Cashier.Id);
                    tw.WriteLine("Cash:  " + Cash+ "€");
                }
        ...
        public static List<Transaction> GetTransactionsbyid(string transactionid)
        {
            var result = new List<Transaction>();
            //using (var cmd = Database.CreateCommand("SELECT * FROM `TransactionsView` WHERE FIX(`date_created`)=@p1"))
            using (var cmd = Database.CreateCommand(Database.GetProvider().QUERY_GET_TRANSACTIONSBy_ID))
            {
                Database.BindParameters(cmd, transactionid);
                Database.GetConnection().Open();
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read()) result.Add(ConvertReaderToTransaction(reader));
