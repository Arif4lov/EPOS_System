...
namespace FastFoodPOS.Forms.AdminForms
{
    public partial class FormTransactions : UserControl
    {
        ...
        void RefreshTransactionsbyid(string transactionId)
        {
            var Transactions = Transaction.GetTransactionsbyid(transactionId);
            DataGridViewTransaction.Rows.Clear();
            Transactions.ForEach((Transaction transaction) =>
            {
                DataGridViewTransaction.Rows.Add(
        ...
        {
            if(TextSearch.Text != "" && TextSearch.Text.Length > 10)
            RefreshTransactionsbyid(TextSearch.Text);
        }
    }
}
